...
function computeCentrelineCutPlan(rawPoints, centrelines, w, oMin) {
  ...
  // The junction. Rather than cutting pieces for the pocket at a fork, the strips already beside it
  // run on through — a longer cut off the same roll — and any piece those long strips have made
  // pointless comes out. Extending and dropping alternate, because each one creates room for the
  // other: a dropped piece frees ground for its neighbours to grow into, and grown neighbours make
  // the next piece redundant. On a floor that is the right trade, and a small corner left bare at a
  // fork is cheaper than a stack of short pieces crossing each other on the ground.
  {
    const EXT_STEP = 0.25;
      ...
      laid.forEach((b, i) => {
        const grow = (sign) => {
          ...
          for (let k = 0; k < 200; k++) {
            ...
            // of it. Demanding all five be clear stopped every strip dead on its own boundary,
            // because a strip's corner meets the neighbouring road's grid before its middle does.
        ...
        b.far = grow(1);
        b.near = grow(-1);
        extentsReach[i] = b.far;
        frontReach[i] = b.near;
        cutLengths[i] = roundToPracticalLength(b.far - b.near, ROUND_STEP);
      });
    };

    // How much of a piece is ground other pieces already cover. Past this it is not earning its cut.
    const REDUNDANT = 0.45;
    const SAMPLE = 0.4;
    const duplicateShare = (i) => {
      const b = laid[i];
      let own = 0, dup = 0;
    ...
    for (let pass = 0; pass < 8; pass++) {
      ...
      for (let i = 0; i < laid.length; i++) {
        const share = duplicateShare(i);
        if (share > worstShare) { worstShare = share; worst = i; }
      }
      if (worst < 0 || worstShare < REDUNDANT || laid.length <= 2) break;
...
function renderCutPlanSvg(svg, cutPlan, w, stripRollNumbers) {
  ...
  cutLengths.forEach((len, i) => {
    ...
    //
    // Sits just OUTSIDE the strip, above its far end. Tried inside the strip once — it was wrong on
    // site, so leave it out here.
    {
      const margin = 6;
      const label = document.createElementNS(ns, "text");
...
function renderCutPlanSvgCornered(svg, cutPlan, w, stripRollNumbers) {
  ...
  cutLengths.forEach((len, i) => {
    ...
    {
      // Just outside the strip, off its far end — same as the flat diagram, and the placement that
      // reads correctly on site.
      const margin = 6;
